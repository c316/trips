...
export const getRaisedTotal = (userId) => {
  ...
  if (Roles.userIsInRole(Meteor.userId(), ['admin', 'leader'])) {
    const thisUser = Meteor.users.findOne({ _id: adminUserId });
    const thisUsersTrip = thisUser.tripId;
    if (thisUser && thisUser.profile && thisUser.profile.firstName) {
      const name = `${thisUser.profile.firstName} ${thisUser.profile.lastName}`;
...
export const getDeadlineAdjustments = (userId) => {
  ...
  } else if (Meteor.user() && Meteor.user.tripId) {
...
export const getSignedURLs = (client, type, file_id) => {
  const image = Images.findOne({ _id: file_id });
  if (image && image._id) {
    if (type === 'thumbnail') {
      const signedThumbnailURL = client.signedUrl(
        image.versions.thumbnail.meta.pipePath,
      ...
      const signedOriginalURL = client.signedUrl(
        image.versions.original.meta.pipePath,
        new Date((new Date().getTime() + 600000)),
      );
