...
(function(factory) {
  ...
  Inputmask.prototype = {
    ...
    format(value, metadata) {
      return maskScope({
        action: 'format',
        value,
        metadata, // true/false getmetadata
      }, this.maskset || generateMaskSet(this.opts, this.noMasksCache), this.opts);
    },
  };
  ...
  function isElementTypeSupported(input, opts) {
    ...
    let isSupported = (input.tagName === 'INPUT' && $.inArray(elementType, opts.supportsInputType) !== -1) || input.isContentEditable || input.tagName === 'TEXTAREA';
  ...
  function maskScope(actionObj, maskset, opts) {
    ...
    function setValidPosition(pos, validTest, fromSetValid, isSelection) {
      if (isSelection || (opts.insertMode && getMaskSet().validPositions[pos] !== undefined && fromSetValid === undefined)) {
        ...
        let positionsClone = $.extend(true, {}, getMaskSet().validPositions),
          lvp = getLastValidPosition(),
          i;
          ...
          if (t !== undefined) {
            ...
            while (posMatch < getMaskSet().maskLength && ((t.match.fn == null && vps[i] && (vps[i].match.optionalQuantifier === true || vps[i].match.optionality === true)) || t.match.fn != null)) {
              ...
              if (needsValidation === false && positionsClone[posMatch] && positionsClone[posMatch].match.def === t.match.def) { // obvious match
                ...
                const result = isValid(posMatch, t.input, true, true);
                valid = result !== false;
    ...
    function stripValidPositions(start, end, nocheck, strict) {
      function IsEnclosedStatic(pos) {
        const posMatch = getMaskSet().validPositions[pos];
    ...
    function determineTestTemplate(tests, ignoreGreedy) {
      let testPos;
      let testPositions = tests,
        lvp = getLastValidPosition(),
        lvTest = getMaskSet().validPositions[lvp] || getTests(0)[0],
        lvTestAltArr = (lvTest.alternation !== undefined) ? lvTest.locator[lvTest.alternation].toString().split(',') : [];
      for (let ndx = 0; ndx < testPositions.length; ndx++) {
    ...
    function selectBestMatch(pos, alternateNdx) {
      ...
      if (getMaskSet().tests[pos] || getMaskSet().validPositions[pos]) {
        $.each(getMaskSet().tests[pos] || [getMaskSet().validPositions[pos]], function(ndx, lmnt) {
          const ndxPos = lmnt.alternation ? lmnt.locator[lmnt.alternation].toString().indexOf(alternateNdx) : -1;
    ...
    function getTests(pos, ndxIntlzr, tstPs) {
      let maskTokens = getMaskSet().maskToken,
      ...
      function resolveTestFromToken(maskToken, ndxInitializer, loopNdx, quantifierRecurse) { // ndxInitializer contains a set of indexes to speedup searches in the mtokens
        function handleMatch(match, loopNdx, quantifierRecurse) {
          function isFirstMatch(latestMatch, tokenGroup) {
            let firstMatch = $.inArray(latestMatch, tokenGroup.matches) === 0;
            if (!firstMatch) {
          ...
          function resolveNdxInitializer(pos, alternateNdx) {
            const bestMatch = selectBestMatch(pos, alternateNdx);
          ...
          if (testPos === pos && match.matches === undefined) {
            ...
            if (match.isGroup && quantifierRecurse !== match) { // when a group pass along to the quantifier
              ...
              const altIndex = ndxInitializer.length > 0 ? ndxInitializer.shift() : -1;
              if (altIndex === -1 || typeof altIndex === 'string') {
                ...
                for (let ndx = 0; ndx < altIndexArr.length; ndx++) {
                  ...
                  if (match !== true && match !== undefined && (altIndexArr[altIndexArr.length - 1] < alternateToken.matches.length)) { // no match in the alternations (length mismatch) => look further
                    const ntndx = $.inArray(match, maskToken.matches) + 1;
                ...
                if (typeof altIndex === 'string') { // filter matches
                  malternateMatches = $.map(malternateMatches, function(lmnt, ndx) {
                    if (isFinite(ndx)) {
                      ...
                      for (let alndx = 0; alndx < altLocArr.length; alndx++) {
    ...
    function isValid(pos, c, strict, fromSetValid, fromAlternate) { // strict true ~ no correction or autofill
      ...
      function _isValid(position, c, strict) {
        ...
        $.each(getTests(position), function(ndx, tst) {
          let test = tst.match,
            loopend = c ? 1 : 0,
            chrs = '';
          for (let i = test.cardinality; i > loopend; i--) {
      ...
      function alternate(pos, c, strict) {
        let validPsClone = $.extend(true, {}, getMaskSet().validPositions),
          lastAlt,
          alternation,
          isValidRslt = false,
          altPos,
        ...
        if (alternation !== undefined) {
          ...
          let decisionTaker = prevAltPos.locator[prevAltPos.alternation || alternation] !== undefined ? prevAltPos.locator[prevAltPos.alternation || alternation] : altNdxs[0]; // no match in the alternations (length mismatch)
          if (decisionTaker.length > 0) { // no decision taken ~ take first one as decider
            decisionTaker = decisionTaker.split(',')[0];
          }
          let possibilityPos = getMaskSet().validPositions[decisionPos],
            prevPos = getMaskSet().validPositions[decisionPos - 1];
          $.each(getTests(decisionPos, prevPos ? prevPos.locator : undefined, decisionPos - 1), function(ndx, test) {
            altNdxs = test.locator[alternation] ? test.locator[alternation].toString().split(',') : [];
            for (let mndx = 0; mndx < altNdxs.length; mndx++) {
              let validInputs = [],
              ...
              if (decisionTaker < altNdxs[mndx] && (test.na === undefined || $.inArray(altNdxs[mndx], test.na.split(',')) === -1)) {
                ...
                const possibilities = getMaskSet().validPositions[decisionPos].locator;
                ...
                if (isValidRslt) {
                  ...
                  const targetLvp = getLastValidPosition(pos) + 1;
      ...
      function trackbackAlternations(originalPos, newPos) {
        let vp = getMaskSet().validPositions[newPos],
      ...
      if (maskPos < getMaskSet().maskLength) {
        ...
        if ((!strict || fromSetValid === true) && result === false) {
          ...
          if (currentPosValid && currentPosValid.match.fn === null && (currentPosValid.match.def === c || c === opts.skipOptionalPartCharacter)) {
            ...
            const testsFromPos = getTests(maskPos).slice();
            if (testsFromPos[testsFromPos.length - 1].match.def === '') testsFromPos.pop();
            let staticChar = determineTestTemplate(testsFromPos, true);
            if (staticChar) {
              staticChar = staticChar.match.placeholder || staticChar.match.def;
              _isValid(maskPos, staticChar, strict);
            }
            for (let nPos = maskPos + 1, snPos = seekNext(maskPos); nPos <= snPos; nPos++) {
    ...
    function seekNext(pos, newBlock) {
      const maskL = getMaskSet().maskLength;
    ...
    function writeBuffer(input, buffer, caretPos, event, triggerInputEvent) {
      if (event && $.isFunction(opts.onBeforeWrite)) {
        const result = opts.onBeforeWrite(event, buffer, caretPos, opts);
        if (result) {
    ...
    function checkVal(input, writeOut, strict, nptvl, initiatingEvent) {
      ...
      function isTemplateMatch() {
        let isMatch = false;
        const charCodeNdx = getBufferTemplate().slice(initialNdx, seekNext(initialNdx)).join('').indexOf(charCodes);
        if (charCodeNdx !== -1 && !isMask(initialNdx)) {
          isMatch = true;
          const bufferTemplateArr = getBufferTemplate().slice(initialNdx, initialNdx + charCodeNdx);
          for (let i = 0; i < bufferTemplateArr.length; i++) {
      ...
      if (!strict) {
        if (opts.autoUnmask !== true) {
          let staticInput = getBufferTemplate().slice(0, seekNext(-1)).join(''),
      ...
      $.each(inputValue, function(ndx, charCode) {
        if (charCode !== undefined) { // inputfallback strips some elements out of the inputarray.  $.each logically presents them as undefined
          ...
          charCodes += charCode;
          let lvp = getLastValidPosition(undefined, true),
            lvTest = getMaskSet().validPositions[lvp],
            nextTest = getTestTemplate(lvp + 1, lvTest ? lvTest.locator.slice() : undefined, lvp);
          if (!isTemplateMatch() || strict || opts.autoUnmask) {
            const pos = strict ? ndx : (nextTest.match.fn == null && nextTest.match.optionality && (lvp + 1) < getMaskSet().p ? lvp + 1 : getMaskSet().p);
      ...
      if (writeOut) {
        let caretPos = (document.activeElement === input && (initiatingEvent || result)) ? (initiatingEvent ? caret(input).begin : result.forwardPosition) : undefined,
    ...
    function unmaskedvalue(input) {
      ...
      let umValue = [],
        vps = getMaskSet().validPositions;
      for (const pndx in vps) {
        if (vps[pndx].match && vps[pndx].match.fn != null) {
          umValue.push(vps[pndx].input);
        }
      }
      let unmaskedValue = umValue.length === 0 ? '' : (isRTL ? umValue.reverse() : umValue).join('');
    ...
    function caret(input, begin, end, notranslate) {
      function translatePosition(pos) {
        if (notranslate !== true && isRTL && typeof pos === 'number' && (!opts.greedy || opts.placeholder !== '')) {
          const bffrLght = getBuffer().join('').length; // join is needed because sometimes we get an empty buffer element which must not be counted for the caret position (numeric alias)
          pos = bffrLght - pos;
        }
        return pos;
      }
      ...
      if (typeof begin === 'number') {
        ...
        const scrollCalc = parseInt(((input.ownerDocument.defaultView || window).getComputedStyle ? (input.ownerDocument.defaultView || window).getComputedStyle(input, null) : input.currentStyle).fontSize) * end;
    ...
    function clearOptionalTail(buffer) {
      let rl = determineLastRequiredPosition(),
      ...
      on(input, eventName, eventHandler) {
        const ev = function(e) {
          ...
          if (this.inputmask === undefined && this.nodeName !== 'FORM') { // happens when cloning an object with jquery.clone
            ...
            const returnVal = eventHandler.apply(this, arguments);
            if (returnVal === false) {
              e.preventDefault();
              e.stopPropagation();
            }
    ...
    function patchValueProperty(npt) {
      ...
      function patchValhook(type) {
        if ($.valHooks && ($.valHooks[type] === undefined || $.valHooks[type].inputmaskpatch !== true)) {
          ...
          const valhookSet = $.valHooks[type] && $.valHooks[type].set ? $.valHooks[type].set : function(elem, value) {
            elem.value = value;
            return elem;
          };
    ...
    function handleRemove(input, k, pos, strict) {
      function generalize() {
        if (opts.keepStatic) {
          ...
          for (; lastAlt >= 0; lastAlt--) {
            const altPos = getMaskSet().validPositions[lastAlt];
            if (altPos) {
      ...
      const lvp = getLastValidPosition(pos.begin, true);
      if (lvp < pos.begin) {
    ...
    function keypressEvent(e, checkval, writeOut, strict, ndx) {
      ...
      if (checkval !== true && (!(e.ctrlKey && e.altKey) && (e.ctrlKey || e.metaKey || ignorable))) {
        ...
        const valResult = isValid(pos, c, strict);
        if (valResult !== false) {
          resetMaskSet(true);
    ...
    function inputFallBackEvent(e) { // fallback when keypress fails
      ...
      if (getBuffer().join('') !== inputValue) {
        let caretPos = caret(input);
        inputValue = inputValue.replace(new RegExp(`(${Inputmask.escapeRegex(getBufferTemplate().join(''))})*`), '');

        if (iemobile) { // iemobile just set the character at the end althought the caret position is correctly set
          const inputChar = inputValue.replace(getBuffer().join(''), '');
        ...
        if ((getBuffer().length - inputValue.length) === 1 && inputValue.charAt(caretPos.begin) !== getBuffer()[caretPos.begin] && inputValue.charAt(caretPos.begin + 1) !== getBuffer()[caretPos.begin] && !isMask(caretPos.begin)) {
          ...
          const lvp = getLastValidPosition() + 1;
          let bufferTemplate = getBuffer().slice(lvp).join('');
          ...
          if (selectedCaret.begin === selectedCaret.end) {
            ...
            default: // lvp:
              ...
              if (clickPosition < lastPosition) {
                ...
                const placeholder = getPlaceholder(lastPosition);
    ...
    function cutEvent(e) {
      ...
      let clipboardData = window.clipboardData || ev.clipboardData,
    ...
    function mask(elem) {
      ...
      if (el.inputmask._valueGet() !== '' || opts.clearMaskOnLostFocus === false || document.activeElement === el) {
        const initialValue = $.isFunction(opts.onBeforeMask) ? (opts.onBeforeMask(el.inputmask._valueGet(), opts) || el.inputmask._valueGet()) : el.inputmask._valueGet();
